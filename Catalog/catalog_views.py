...
async def catalog_picture(id: int):
    ...
    print(type(res.picture))
    print(f"Type: {type(res.picture)}")
    print(f"Length: {len(res.picture)}")
        
        # Проверяем первые несколько байт (должны быть сигнатура JPEG)
    print(f"First bytes: {res.picture[:4].hex()}")
    print(f"As string: {res.picture.decode('utf-8', errors='ignore')}")
    return Response(content=res.picture, media_type="image/jpeg")
